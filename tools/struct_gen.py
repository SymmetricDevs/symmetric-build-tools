        ...
        if not self.enum:
            ...
            for i in self.members.items():
                ...
                if i[0].lower() == i[0] and i[0] != "type":
                    ...
                    out += f"    #[serde(rename = \"{i[0]}\")]\n    {struct_change_case(i[0])}: {i[1][0]},\n"

            out += "}"
            return out
        else:
            out = f"#[derive(Serialize_repr, Deserialize_repr, PartialEq, Debug)]\n#[repr(u8)]\npub enum {self.name} {chr(123)}\n"
            for i in self.members.items():
                out += f"    {enum_change_case(i[1])} = {i[0]},\n"
